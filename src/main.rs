...
use lexx::token::{Token, TOKEN_TYPE_FLOAT, TOKEN_TYPE_INTEGER, TOKEN_TYPE_KEYWORD, TOKEN_TYPE_SYMBOL, TOKEN_TYPE_WHITESPACE};

use crate::compiler::{Block, CompileContext, CompileError, Compiler, COMPILER_TYPE_FLOAT, COMPILER_TYPE_IDENTIFIER, COMPILER_TYPE_INT, COMPILER_TYPE_LET, COMPILER_TYPE_MATH, COMPILER_TYPE_NEGATE, COMPILER_TYPE_TERNARY, COMPILER_TYPE_TYPE_DECLARATION, CompileUml, FloatCompiler, IdentifierCompiler, IntCompiler, LetCompiler, MathCompiler, NegateCompiler, TernaryCompiler, TypeCompiler, TypeOperatorCompiler};
...
pub fn make_parse_context(input: Box<dyn LexxInput>) -> ParseContext {
  ...
  let type_parser = PrefixParslet {
    matcher: |_ctx, token| {
      if token.token_type == TOKEN_TYPE_SYMBOL && "<" == token.value { true } else { false }
    },
    generator: |ctx, token| {
      let right = Parser::parse(ctx, &None, PRECEDENCE_EXPONENT)?;
      ...
      match right {
        ...
        Some(r) => {
          let t = r.as_ref().borrow().get_type();
          if t != COMPILER_TYPE_TYPE_DECLARATION && t != COMPILER_TYPE_IDENTIFIER {
            return Err(ParseError::Error(format!("Cannot parse Type '{}' at {}, {}", token.value, token.line, token.column).parse().unwrap()));
          }
          Ok(Some(Rc::new(RefCell::new(TypeCompiler {
            right: Some(r),
            token: token.clone(),
...
pub fn run_script(parse_context: &mut ParseContext) -> Result<ExecutionContext, MojoError> {
  ...
  let compile_result = compiler.borrow().compile(&mut CompileContext {
    namespace: Rc::new(RefCell::new(Block {
      names: HashMap::new(),
      parent: None,
    }))
  }, None)?;
...
pub fn collect_uml(parse_context: &mut ParseContext) -> Result<String, MojoError> {
  ...
  let comp_ctx = &mut CompileContext {
    namespace: Rc::new(RefCell::new(Block {
      names: HashMap::new(),
      parent: None,
    }))
...
mod tests {
  ...
  #[test]
  fn test_type_uml() {
    let mut pc = make_parse_context(Box::new(InputString::new(String::from("let test: int"))));
    let uml = collect_uml(&mut pc);
    println!("{}", uml.unwrap());
  }
}